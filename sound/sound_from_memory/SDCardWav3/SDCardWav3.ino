...
//  I2S configuration
      ...
      {
          .bck_io_num = 27,//I2S_BCLK,                           // The bit clock connectiom, goes to pin 27 of ESP32
          .ws_io_num = 26,//I2S_LRC,                             // Word select, also known as word select or left right clock
          .data_out_num = 25,//I2S_DOUT,                         // Data out from the ESP32, connect to DIN on 38357A
...
void setup() {    
    ...
    WavFile = SPIFFS.open(song_name, "r");                   // Open the wav file
    if(WavFile==false)
...
uint16_t ReadFile(byte* Samples)
{
    ...
    if(BytesReadSoFar>=WavHeader.DataSize)              // Have we read in all the data?
    {
      ...
      delay(2000);
      song_name = "/incorrec_song.wav";
      WavFile = SPIFFS.open(song_name, "r");
      delay(2000); 
    }
